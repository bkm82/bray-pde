...
$\frac{\partial T}{\partial t} = \frac{T ^{n+1}-T ^{n}}{\Delta t} + \mathcal{O}(\Delta t)$

and the spatial second derivative can be discritized using a centeral difference scheme

$\frac{\partial ^2 T}{\partial x ^2} = \frac{T _{n-1}-2T _{n}+T _{n+1}}{(\Delta x)^2} + \mathcal{O}((\Delta x)^2)$
**** Boundarys
At the right hand boundary the $T_{n+1}$ must be solved for. For a Neuiman boundary condtion the flux is known

$\frac{\partial T}{\partial x } = q _ {right}$

can be discritized

Right Neuimann Boundary condition
...
Create a solver object with time paramaters, desired method, and a mesh and solve using .solve(t_final)
#+ATTR_LATEX: :options frame=single
#+BEGIN_SRC python :results output :exports both :session Python-Session
